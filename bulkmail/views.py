...
import json
import traceback
from bulkmail.mailers.mailgun import mailgun_send
...
from .shortcuts import render_tpl, ok, oops
...
@csrf_exempt
def mailgun_bouncer (request):
  #todo: add signature verificiation
  raw = request.body
  try:
    if 'event' in request.REQUEST:
      if request.REQUEST['event'] == 'bounced':
        b = Bounce(btype='bounce', original_to=request.REQUEST['recipient'], raw_message=raw)
        b.put()
      ...
      elif request.REQUEST['event'] == 'complained':
        b = Bounce(btype='complaint', original_to=request.REQUEST['recipient'], raw_message=raw)
        b.put()
